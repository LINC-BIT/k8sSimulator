...
func (f *Fit) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	//for resource := range f.resourceToWeightMap {
	//	alloc, req := f.calculateResourceAllocatableRequest(nodeInfo, pod, resource)
	//	if alloc != 0 {
	//		// Only fill the extended resource entry when it's non-zero.
	//		allocatable[resource], requested[resource] = alloc, req
	//	}
	//}
	//fmt.Println("r,a:",requested,allocatable,f.scorer(requested,allocatable))
