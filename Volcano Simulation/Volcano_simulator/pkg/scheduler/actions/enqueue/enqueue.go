...
func (enqueue *Action) Execute(ssn *framework.Session) {
	...
	//这里的queue、queueMap、jobsMap是方法中临时的，会用到它们为空
	queues := util.NewPriorityQueue(ssn.QueueOrderFn)
	queueMap := map[api.QueueID]*api.QueueInfo{}
	jobsMap := map[api.QueueID]*util.PriorityQueue{} //jobsMap的key为job属于的queue id
	...
	for {
		...
		// Found "high" priority job，从最高级queue的最高级job遍历起
		...
		//资源充足则入ssn的队
		if job.PodGroup.Spec.MinResources == nil || ssn.JobEnqueueable(job) { //用到overcommit
			ssn.JobEnqueued(job)
