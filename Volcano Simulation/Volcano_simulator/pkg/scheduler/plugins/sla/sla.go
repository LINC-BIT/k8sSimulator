...
*/
func (sp *slaPlugin) OnSessionOpenV0(ssn *framework.Session) {
...
func (sp *slaPlugin) OnSessionOpen(ssn *framework.Session) {
	...
	jobOrderFn := func(l, r interface{}) int {
		...
		var lJobWaitingTime = sp.readJobWaitingTime(lv.WaitingTime) //若job设置了WaitingTime，则返回该WaitingTime，否则返回schedulerCfg中声明的slaWaitingTime
		var rJobWaitingTime = sp.readJobWaitingTime(rv.WaitingTime)

		//nil表示sla-waiting-time为无限
		if lJobWaitingTime == nil {
			if rJobWaitingTime == nil {
				return 0
			}
	...
	permitableFn := func(obj interface{}) int {
		...
		//	return util.Abstain
		//}
		if api.NowTime.Sub(jobInfo.CreationTimestamp.Time) <*jwt{ //还在job waiting time范围里
			return util.Abstain
		}
