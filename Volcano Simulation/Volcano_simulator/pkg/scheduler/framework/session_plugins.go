...
// JobValid invoke jobvalid function of the plugins，用ssn所有jobValidFns处理job，返回nil表示job是valid的，否则会返回result
...
func (ssn *Session) NodeOrderMapFn(task *api.TaskInfo, node *api.NodeInfo) (map[string]float64, float64, error) {
	...
	for _, tier := range ssn.Tiers {
		for _, plugin := range tier.Plugins {
			...
			if pfn, found := ssn.nodeOrderFns[plugin.Name]; found { //实际打分
				score, err := pfn(task, node)
				if err != nil {
