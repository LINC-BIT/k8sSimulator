...
var NowTime = metav1.NewTime(time.Time{})
...
type TaskInfoV0 struct {
	...
	//和numa-aware、task-topology两个plugin有关
...
// TaskInfo will have all infos about the task
type TaskInfo struct { //修改TaskInfo还要改Clone()方法
	UID TaskID
	Job JobID
	...
	//自己加的，在给task分配了request cpu后若还有剩余cpu，则还会继续分配
	ActualCpu float64

	//自己加的，container创建倒计时
	CtnCreationCountDown float64

	//自己加的，用于在异步时记录pod工作量
	Workload float64

	//自己加的，用于在异步时记录pod工作多久未完成则重启,-1则表示无重启时间
	RestartTime float64

	//自己加的，用于在异步时记录pod工作多久未完成则终止,-1则表示无终止时间
...
func NewTaskInfo(pod *v1.Pod) *TaskInfo {
	...
	jobID := getJobID(pod) //用到v1beta1.KubeGroupNameAnnotationKey
...
func (ti *TaskInfo) CloneV0() *TaskInfo {
	return &TaskInfo{
		UID:           ti.UID,
		Job:           ti.Job,
		Name:          ti.Name,
...
func (ti *TaskInfo) Clone() *TaskInfo {
	return &TaskInfo{
		...
		LastTransaction: ti.LastTransaction.Clone(),
		SimEndTimestamp: ti.SimEndTimestamp, //自己加的
		CtnCreationCountDown: ti.CtnCreationCountDown,
		Workload: ti.Workload,
		RestartTime: ti.RestartTime,
		TerminationTime: ti.TerminationTime,
	}
}
...
type JobInfoV0 struct {
	...
	//sla等待时间，可以由JobInfo.SetPodGroup用sla标签内容填充，也可以自己填充，不初始化则为nil
	...
	//Allocated指job假设被分配到的资源数
	Allocated    *Resource
	//Allocated指job需要被分配到的资源数
	TotalRequest *Resource

	//metav1.Time类型不初始化就是0001-01-01 00:00:00 +0000 UTC（而非nil）
...
type JobInfo struct {
	...
	//自己加的
	SubTimestamp metav1.Time

	//自己加的，用于在异步时记录pod工作多久未完成则终止,目前还可以终止的pod数,-1表示无上限
	TerminationNum float64
...
func NewJobInfoV2(job *batch.Job) *JobInfo{
	jobInfo := &JobInfo{
		UID: JobID(""), //之后再设置
		Name: job.ObjectMeta.Name,
		Namespace: job.ObjectMeta.Namespace,
		Queue : QueueID(job.Spec.Queue),
		Priority: int32(1),
		MinAvailable: job.Spec.MinAvailable,
		//WaitingTime//由JobInfo.SetPodGroup用sla填充
		NodesFitErrors:   make(map[TaskID]*FitErrors),
		TaskStatusIndex:  map[TaskStatus]tasksMap{},
		Tasks:            tasksMap{}, //AddTaskInfo填充
		TaskMinAvailable: map[TaskID]int32{},
	...
	if _, exist := job.Annotations[JobWaitingTime]; exist {
		if jobWaitingTime, err := time.ParseDuration(job.Annotations[JobWaitingTime]);err==nil{
			if jobWaitingTime > 0{
				jobInfo.WaitingTime = &jobWaitingTime
			}
		}
	}

	if jobInfo.Queue == QueueID(""){
		jobInfo.Queue = QueueID("default")
	}
	if jobInfo.Namespace == ""{
		jobInfo.Namespace = "default"
	}
	...
	for i:=int32(0);i<job.Spec.Tasks[0].Replicas;i++{
		pod:=&v1.Pod{
			...
			Status: v1.PodStatus{
				Phase: v1.PodPending,
				//Phase: v1.PodRunning, //test
			},
			Spec: job.Spec.Tasks[0].Template.Spec,
		}
		//pod.Labels["job"] = string(jobInfo.UID) //将job名作为标签
...
// AddTaskInfo is used to add a task to a job, change JobInfo.Tasks,JobInfo.TotalRequest and JobInfo.Allocated
...
func (ji *JobInfo) UpdateTaskStatusV0(task *TaskInfo, status TaskStatus) error {
	...
	// Update task's status to the target status once task addition is guaranteed to succeed.删除后再重新添加
	task.Status = status
	ji.AddTaskInfo(task)
...
func (ji *JobInfo) UpdateTaskStatus(task *TaskInfo, status TaskStatus) error {
	...
	task.Status = status
	if status == Binding{
		task.Pod.Status.StartTime = NowTime.DeepCopy()
		task.Pod.Status.ContainerStatuses = []v1.ContainerStatus{
			{ //ContainerStatus[0]
				State: v1.ContainerState{
					Terminated: &v1.ContainerStateTerminated{
						StartedAt: metav1.NewTime(NowTime.Local()),
					},
		...
		if simTime, found := task.Pod.Labels["sim-time"]; found {
			if timestamp,err := strconv.Atoi(simTime); err == nil {
				task.SimEndTimestamp = metav1.NewTime(task.Pod.CreationTimestamp.Add(time.Duration((timestamp+rand.Intn(1))*1e9))) //加了随机数表示pod创建时间
			}
		}
	}
	ji.AddTaskInfo(task)
...
func (ji *JobInfo) CloneV0() *JobInfo {
	info := &JobInfo{
		UID:       ji.UID,
		Name:      ji.Name,
...
func (ji *JobInfo) Clone() *JobInfo {
	info := &JobInfo{
		...
		Preemptable:           ji.Preemptable,
		RevocableZone:         ji.RevocableZone,
		//Budget:                ji.Budget.Clone(), //注释掉了
		TerminationNum:        ji.TerminationNum,
	}
