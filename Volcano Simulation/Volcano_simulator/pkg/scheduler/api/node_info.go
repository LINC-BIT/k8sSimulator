...
type NodeInfo struct {
	...
	//Allocatable指node节点所有资源（Capability）中可以用于调度的部分，不会变化
	Allocatable *Resource
	//Capability指node节点所有资源
	Capability  *Resource
	...
	//自己加的，总cpu减去request cpu后的剩余值
	CpuTotal float64
	CpuReq float64
	CpuLimits float64

	//自己加的，node计算速度，如0.8表示一个node完成一个负载的时间为正常时间的1/0.8倍
	CalculationSpeed float64

	//自己加的，node计算速度会下降，表示该值最低值,-1表示无
	MinimumSpeed float64

	//自己加的，node计算速度会下降，表示cpu占用率达到多少时开始下降,-1表示无
	SlowSpeedThreshold float64

	//自己加的，表示某个时间段的使用cpu的计算速率
	CpuCalculationSpeed float64

	//自己加的，在该node上创建一个container需要的时间
	CtnCreationTime float64

	//自己加的，在该node上多创建一个container需要的额外时间，如创建3个，则第一个container创建时间为 CtnCreationTime+2*CtnCreationExtraTime
	CtnCreationExtraTime float64

	//自己加的，在该node上多创建一个container需要的额外时间，如创建3个，则第一个container创建时间为 CtnCreationTime+2*CtnCreationExtraTime ，第二个为 CtnCreationTime+2*CtnCreationExtraTime+CtnCreationTimeInterVal
	CtnCreationTimeInterval int64
}
...
func NewNodeInfo(node *v1.Node) *NodeInfo {
	...
	nodeInfo.setNodeGPUInfo(node) //需要用到capacity
...
func (ni *NodeInfo) AddTask(task *TaskInfo) error {
	...
	if ni.Node != nil {
		switch ti.Status {
		case Releasing: //task的pod被evict了
