...
# 1. 介绍
K8sSim，是一种用于Kubernetes集群调度及其在调度算法优化方面的的仿真工具。该模拟器结合了通用负载和AI工作负载的典型Kubernetes调度算法和Volcano调度算法，并在实际集群中提供了对运行job和task调度过程的准确模拟。它具有两个优点：（1）可以确保调度结果的准确性；（2）还可以大大加速实际群集中不同调度算法的调度运行时间。 因此，K8sSim可以为一系列有关集群计划优化的研究工作提供方便的分析工具和基础。
